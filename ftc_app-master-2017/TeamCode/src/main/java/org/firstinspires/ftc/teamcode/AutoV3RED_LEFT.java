...
@Autonomous(name="AutoV3RED_LEFT", group="Red Linear Opmode")

public class AutoV3RED_LEFT extends LinearOpMode {
    ...
    public void runOpMode()
    {
        ...
        arm.setPosition(0.3);
        while (Math.abs(imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES).firstAngle) >= 3 && !isStopRequested())
        {
        ...
        while (Math.abs(imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES).firstAngle) <= 177 && !isStopRequested());
